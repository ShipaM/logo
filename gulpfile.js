let project_folder = require("path").basename(__dirname); //папка продакшена для заказчика
...
let path = {
    build: { //пути вывода куда gulp будет выгружать уже обработанные файлы
        html: project_folder + "/",
    ...
    watch: { //будем слушать эти файлы и моментально делать изменения
        html: source_folder + "/**/*.html",
        css: source_folder + "/sass/**/*.sass",
        js: source_folder + "/js/**/*.js",
        img: source_folder + "/img/**/*.{jpg,png,svg,gif,ico,webp}",
    },
    clean: "./" + project_folder + "/" //будем удалять при каждом запуске gulp
}
...
function browserSync(params) { //запускает локальный сервер
    browsersync.init({
        server: {
...
//gulp svgSprite для зауска таска единоразово
